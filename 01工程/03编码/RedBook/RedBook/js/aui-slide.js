...
(function(window) {
	...
	auiSlide.prototype.touchEnd = function(event) {
		...
		var boundary = this.slideWrapWidth/100;
		var duration = (new Date() * 1) - touchStartTime;
		var isValid = Number(duration) < 250 && Math.abs(offsetX) > 5 || Math.abs(offsetX) > boundary;
		if (isScrolling) {
			return;
		}
        if(isValid){
