<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="user.quanconfirm.aspx.cs" Inherits="RedBook.user_QuanConfirm" %>
...
<body>
    <header class="header">
        <h2>直购商品确认</h2>
        <a class="cefenlei" href="/user.zhigou.aspx">
    ...
    <section class="genzong-box">
        <div class="gztt">商品跟踪</div>
        <ul class="genzong-listbox">
        </ul>
    </section>
    <section style="height: 45px;"></section>

    <section class="fixbox" style="display:none">
        <div class="fixnr"><span>感谢您的购买！</span><!--<a class="link-canyu" href="gift.index.aspx">继续参与</a>--></div>
    </section>
    ...
    <script type="text/javascript">
        ...
        function GetGoodsInfo() {
            var ajaxUrl = "http://" + window.location.host + "/ilerrshareorder?action=quanConfirm&orderid=" + orderid ;
            //alert(ajaxUrl);
            var goodsinfo = "",
                status = "";
            $.ajax({
        ...
        function GetHuafeiStep2(e) {
            ...
            var GetHuafeili = '<li id="yipaifa">' +
               ...
               // '<a class="arrtxt blue" href="/user.chongzhiInfo.aspx">卡密使用说明<i class="z-arrow"></i></a>' +
                ...
                //'</p>' +
                '<div class="kuang">' +
                    '<div class="knr">卡号：' + e.kahao.replace(e.kahao.substring(6, 10), "****") + '</div>' +
                    '<div class="knr">密码：' + e.kami.replace(e.kahao.substring(6, 10), "****") + '</div>' +
                '</div>' +
            '</li>' +
            '<li id="ShiyongWay">' +
                    ...
                    if (addr.default == "Y") {//有默认收货地址
                        for (var i = 0; i < addr.data.length; i++) {
                            if (addr.data[i].isdefault == "Y") {
                                li = '<li>' +
                                    '<h1>请确认收货地址</h1>' +
                                    '<a id="AddrKuang" class="kuang" href="/user.address.aspx?orderid=' + orderid + '&shopid=' + shopid + '&productid=' + productid + '&pageFrom=quanConfirm">' +
        ...
        function GetShouhuoStep4(e) {
            ...
            '</li>' +
            '<li>' +
                '<h1>您的奖品已签收<time>' + e.time + '</time></h1>' +
                '<p>祝您再中大奖~</p>' +
            '</li>';
        }
        ...
        function GetGameStep4(e) {
            ...
            '</li>' +
            '<li>' +
                '<h1>您的物品已签收<time>' + e.time + '</time></h1>' +
                '<p>祝您再中大奖~</p>' +
            '</li>';
        }
