...
<body>
    ...
    <script type="text/javascript">
        ...
        function getGiftList() {
            var ajaxUrl = "http://" + window.location.host + "/ilerrgift?action=giftWinDetail";
            $.ajax({
                ...
                success: function (data) {
                    if (data.length > 0) {
                        for (var i = 0; i < data.length; i++) {
                            ...
                            html += '</div>'
                            html += '<p class="txt">积分：' + data[i].score + '<b>' + data[i].status + '</b></p>'
                            html += '<p class="txt">' + data[i].time + '</p>'
                            html += '<span class="gongxi"></span>'
                            html += '</div>'
                            html += '</li>'
                            html += '</ul>'
        ...
        function goConfirm(orderId) {
            window.location = "/user.giftconfirm.aspx?&orderid=" + orderId;
        }
    </script>
</body>
</html>
