              ...
              $('#step3').click(function(){
                register(function(data){
                  ...
                  if(data.code === 0){
                    ...
                    showAlertDiv('验证码填写错误');
                    $('.step3').hide();
                    $('.step2').show();
                    $('#nav_title').text('填写验证码');
                  }
                  else {
                    showAlertDiv(data.msg);
                    $('.step3').hide();
                    $('.step2').show();
                    $('#nav_title').text('填写验证码');
                  }
            ...
            (function(){
              ...
              $('#get_smscode').on('click', function(){
                $.get('/user/send_resetpwd_sms', { phone: $('input[name=phone]').val() }, function(data){
                  ...
                  if(data.code === 0){
                      showAlertDiv('短信已发出，请稍候');
                  }
                  else if(data.code === 5){
                      showAlertDiv('短信发送太频繁，请稍候再试');
                  }
                  else {
                      showAlertDiv('手机号码错误');
                  }
          ...
          if( $('.db-main-cart,.db-main-order').length ) {
            ...
            (function(){
              $('.subtract').click(function(){
                ...
                if( number<=price_tag ) {
                    showAlertDiv('亲，数量至少为' + price_tag + '哦~');
                } else {
