...
namespace YH.Weixin.MP.Api
{
    ...
    {
        private const string TEMP_JSON_DATA = @"{""expire_seconds"":1800, ""action_name"": ""QR_SCENE"", ""action_info"": {""scene"": {""scene_id"":""{0}""}}}";//只能接受整型
        private const string FIXED_JSON_DATA = @"{""action_name"": ""QR_LIMIT_SCENE"", ""action_info"": {""scene"": {""scene_id"":""{0}""}}}";//只能接受整型
        ...
        public static string CreateTicket(string WeixinAppId, string WeixinAppSecret, string ticketID, bool isTemp = true)
        {
            ...
            //WxLogger(json);
            string strTICKET = new Weixin.MP.QrCode.WebUtils().DoPost(url, json);
        ...
        /// 获取二维码
        /// </summary>
        public static string GetTicketImage(string WeixinAppId, string WeixinAppSecret, string ticketID, bool isTemp = true,string writeInfo=null)
        {
            string TICKET = CreateTicket(WeixinAppId, WeixinAppSecret,ticketID, isTemp);
            return GetTicketImage(TICKET, ticketID, writeInfo);
        }

        /// <summary>
        /// 获取二维码
        /// </summary>
        public static string GetTicketImage(string TICKET, string ticketID, string writeInfo = null)
        {
            ...
            using (WebResponse wr = req.GetResponse())
            {
                ...
                if (!string.IsNullOrEmpty(writeInfo)) ticketID = writeInfo;
                savepath = HttpContext.Current.Server.MapPath("~/Resources/TicketImage") + "\\" + ticketID + "." + myResponse.ContentType.Split('/')[1].ToString();
                ...
                try
                {
                    ...
                    {
                        ImageHelper.WriteFont(savepath, writeInfo);
                        //靳华斌_13349946975
                    }
                }
                catch (Exception ex)
