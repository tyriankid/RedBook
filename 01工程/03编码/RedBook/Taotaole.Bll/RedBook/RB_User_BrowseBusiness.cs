...
namespace Taotaole.Bll
{
	...
	/// -业务操作类
	/// </summary>
	public class RB_User_BrowseBusiness
	{
		/// <summary>
		/// 根据主键加载数据集
		/// </summary>
		public static DataTable LoadData(Guid ID, DbServers.DbServerName currDbName = DbServers.DbServerName.LatestDB)
		{
			...
			{
				return RB_User_BrowseManager.LoadData(ID, currDbName);
			}
		}
		...
		{
				return RB_User_BrowseManager.LoadEntity(ID, currDbName);
		}
		...
		{
			return RB_User_BrowseManager.SelectListData(where,selectFields,orderby,top, currDbName);
		}
		...
		{
			return RB_User_BrowseManager.SelectScalar(where,selectFields,orderby, currDbName);
		}

		/// <summary>
		/// 根据条件查询数据实体
		/// </summary>
		public static IList<RB_User_BrowseEntity> GetListEntity(string where = null, string selectFields ="*", string orderby = null, int top = 0, DbServers.DbServerName currDbName = DbServers.DbServerName.LatestDB)
		{
				return RB_User_BrowseManager.SelectListEntity(where,selectFields,orderby,top, currDbName);
		}
		...
		{
			RB_User_BrowseManager.Del(ID, currDbName);
		}
		...
		{
			RB_User_BrowseManager.DelListData(where, currDbName);
		}
		...
		{
			return RB_User_BrowseManager.SaveEntity(entity, isAdd, currDbName);
		}
