...
namespace Taotaole.Bll
{
	...
	/// -业务操作类
	/// </summary>
	public class RB_User_FocusBusiness
	{
		/// <summary>
		/// 根据主键加载数据集
		/// </summary>
		public static DataTable LoadData(Guid ID, DbServers.DbServerName currDbName = DbServers.DbServerName.LatestDB)
		{
			...
			{
				return RB_User_FocusManager.LoadData(ID, currDbName);
			}
		}
		...
		{
				return RB_User_FocusManager.LoadEntity(ID, currDbName);
		}
		...
		{
			return RB_User_FocusManager.SelectListData(where,selectFields,orderby,top, currDbName);
		}
		...
		{
			return RB_User_FocusManager.SelectScalar(where,selectFields,orderby, currDbName);
		}

		/// <summary>
		/// 根据条件查询数据实体
		/// </summary>
		public static IList<RB_User_FocusEntity> GetListEntity(string where = null, string selectFields ="*", string orderby = null, int top = 0, DbServers.DbServerName currDbName = DbServers.DbServerName.LatestDB)
		{
				return RB_User_FocusManager.SelectListEntity(where,selectFields,orderby,top, currDbName);
		}
		...
		{
			RB_User_FocusManager.Del(ID, currDbName);
		}
		...
		{
			RB_User_FocusManager.DelListData(where, currDbName);
		}
		...
		{
			return RB_User_FocusManager.SaveEntity(entity, isAdd, currDbName);
		}
