...
namespace Taotaole.Bll
{
	...
	/// -业务操作类
	/// </summary>
	public class RB_User_DynamicBusiness
	{
		/// <summary>
		/// 根据主键加载数据集
		/// </summary>
		public static DataTable LoadData(Guid ID, DbServers.DbServerName currDbName = DbServers.DbServerName.LatestDB)
		{
			...
			{
				return RB_User_DynamicManager.LoadData(ID, currDbName);
			}
		}
		...
		{
				return RB_User_DynamicManager.LoadEntity(ID, currDbName);
		}
		...
		{
			return RB_User_DynamicManager.SelectListData(where,selectFields,orderby,top, currDbName);
		}
		...
		{
			return RB_User_DynamicManager.SelectScalar(where,selectFields,orderby, currDbName);
		}

		/// <summary>
		/// 根据条件查询数据实体
		/// </summary>
		public static IList<RB_User_DynamicEntity> GetListEntity(string where = null, string selectFields ="*", string orderby = null, int top = 0, DbServers.DbServerName currDbName = DbServers.DbServerName.LatestDB)
		{
				return RB_User_DynamicManager.SelectListEntity(where,selectFields,orderby,top, currDbName);
		}
		...
		{
			RB_User_DynamicManager.Del(ID, currDbName);
		}
		...
		{
			RB_User_DynamicManager.DelListData(where, currDbName);
		}
		...
		{
			return RB_User_DynamicManager.SaveEntity(entity, isAdd, currDbName);
		}
