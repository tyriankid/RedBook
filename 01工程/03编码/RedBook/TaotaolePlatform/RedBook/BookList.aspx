<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="BookList.aspx.cs" Inherits="RedBookPlatform.RedBook.BookList" %>
		...
		<div class="wrapper">
			<div class="crumbs">
				<ul id="breadcrumbs" class="breadcrumb"> 
	                ...
	                <li class="active"><a href="#" title="">导购文章列表</a></li>
	            </ul>
			</div>
		    <div class="content">
		    	<div class="oprationbtn">
		    		<a class="btn" href="javascript:;" onclick="openNewWin('/RedBook/addBook.aspx')">添加导购文章</a>
		    	...
		    	<table class="table table-striped table-even">
                    ...
                    <tbody>
                        ...
                        <ItemTemplate>
                            <tr>
                                <td style="width:80px;text-align:center"><input type="text" name="ordersn" id="ordersn" value='<%#Eval("SortBaseNum") %>' onchange="uporders(<%#Eval("id") %>,this)" size="2" /></td>
                                <td><%#Eval("id") %></td>
                                <td><div class="imgBox"><img src="<%=G_UPLOAD_PATH %><%#Eval("BackImgUrl") %>"/></div><%#Eval("mainTitle") %></td>
                                <td><%#Eval("SubTitle") %></td>
                                <td>点赞数：<%#Eval("FabulousCount") %> 收藏数：<%#Eval("FavoriteCount") %> 观看数：<%#Eval("WatchCount") %></td>
                                <td><%#Eval("CategoryName") %></td>
                                <td style="width:120px;text-align:center" class="opration"><span onclick="openNewWin('/RedBook/addBook.aspx?id=<%#Eval("id") %>')">修改</span> | <span onclick="goDelete('<%#Eval("mainTitle") %>','<%#Eval("id") %>')">删除</span></td>
                            </tr>
                        </ItemTemplate>
                    </asp:Repeater>
                    </tbody>
