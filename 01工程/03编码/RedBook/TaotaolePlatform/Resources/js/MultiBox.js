...
//图文json对象
var tws = [];
var tw = function () { return { "BoxId": "", "Title": "", "Url": "", "Description": "", "Content": "", "Imgurl": "", "Status": "" } }
ntw = new tw();
ntw.BoxId = 1;
tws.push(ntw);
ntw = new tw();
...
function saveDataToJson() {
    ...
    for (var a in tws) {
        ...
        if (ia + 1 == boxIdN) {
            ...
            tws[ia].Content = editor.getContent();
            tws[ia].Url = $("#urlData").val();
            tws[ia].Imgurl = $("#fmSrc").val();
        }
    }
...
function loadData() {
    ...
    $("#fmSrc").val(tws[boxIdN - 1].Imgurl);
    $("#urlData").val(tws[boxIdN - 1].Url);
    //editor.setContent(tws[boxIdN - 1].Content);
    if (tws[boxIdN - 1].Imgurl != "" && tws[boxIdN - 1].Imgurl != null) {
        var smallimg = $("<img>");
        var delimg = "<a id=\"delpic\" href='javascript:void(0)' onclick='RemovePicMulti()'>删除</a>";
        $("#smallpic").empty();
        smallimg.attr("src", tws[boxIdN - 1].Imgurl);
