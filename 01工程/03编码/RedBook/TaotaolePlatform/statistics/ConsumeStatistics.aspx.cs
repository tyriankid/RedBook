...
{
    public partial class ConsumeStatistics : Base
    {
        ...
        private void BindPage()
        {
            ...
            //调用通用分页方法绑定
            string from = "( select t.*,m.username,m.time,m.mobile,m.email,m.money as moneynow from (select uid, sum(money) as money  from go_member_account group by uid ) t join go_member m on t.uid=m.uid  where m.wxid not like '%test%' and typeid!=9 )t ";
