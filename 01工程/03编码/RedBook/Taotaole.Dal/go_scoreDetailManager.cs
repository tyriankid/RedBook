...
namespace Taotaole.Dal
{
	...
	/// -数据库操作类
	/// </summary>
	public class go_scoreDetailManager
	{
		...
		{
			string selectSql = string.Format(@"Select * From go_scoreDetail Where ScoreId={0}", ID);
			DataSet ds = DataAccessFactory.GetDataProvider(DbServers.GetCurrentDB(currDbName)).GetDataset(selectSql);
			ds.Tables[0].TableName = "go_scoreDetail";
			ds.Tables[0].PrimaryKey = new DataColumn[] { ds.Tables[0].Columns["ScoreId"] };
			return ds.Tables[0];
		}
		...
		public static go_scoreDetailEntity LoadEntity(int ID, DbServers.DbServerName currDbName = DbServers.DbServerName.LatestDB)
		{
			...
			{
				return ReaderConvert.ReaderToModel<go_scoreDetailEntity>(reader);
			}
		}
		...
		public static DataTable SelectListData(string where = null, string selectFields ="*", string orderby = null, int top = 0, DbServers.DbServerName currDbName = DbServers.DbServerName.LatestDB)
		{
			...
			string selectSql = string.Format(@"Select {2} {1} From go_scoreDetail {0}", where, selectFields, top == 0 ? "" : "top " + top);
		...
		{
			if (!string.IsNullOrEmpty(where)) where = " Where " + where;
			string selectSql = string.Format(@"Select {1} From go_scoreDetail {0}", where, selectFields);
			if (!string.IsNullOrEmpty(orderby)) selectSql += " Order By " + orderby;
		...
		/// 根据条件查询数据实体
		/// </summary>
		public static IList<go_scoreDetailEntity> SelectListEntity(string where = null, string selectFields ="*", string orderby = null, int top = 0, DbServers.DbServerName currDbName = DbServers.DbServerName.LatestDB)
		{
			if (!string.IsNullOrEmpty(where)) where = " Where " + where;
			string selectSql = string.Format(@"Select {2} {1} From go_scoreDetail {0}", where, selectFields, top == 0 ? "" : "top " + top);
			if (!string.IsNullOrEmpty(orderby)) selectSql += " Order By " + orderby;
			using (IDataReader reader = DataAccessFactory.GetDataProvider(DbServers.GetCurrentDB(currDbName)).GetReader(selectSql))
			{
				return ReaderConvert.ReaderToList<go_scoreDetailEntity>(reader);
			}
		}
		...
		{
			string deleteSql = string.Format(@"Delete From go_scoreDetail Where ScoreId={0}", ID);
		...
		{
			if (!string.IsNullOrEmpty(where)) where = " Where " + where;
			string deleteSql = string.Format(@"Delete From go_scoreDetail {0}", where);
			DataAccessFactory.GetDataProvider(DbServers.GetCurrentDB(currDbName)).Execute(deleteSql);
		}
		...
		public static bool SaveEntity(go_scoreDetailEntity entity, bool isAdd, DbServers.DbServerName currDbName = DbServers.DbServerName.LatestDB)
		{
			try
			{
				...
				"Insert Into go_scoreDetail(uid,money,usetime,detail,use_range,type,orderId)values(@uid,@money,@usetime,@detail,@use_range,@type,@orderId)" :
                "Update go_scoreDetail Set uid=@uid,money=@money,usetime=@usetime,detail=@detail,use_range=@use_range,type=@type,orderId=@orderId Where ScoreId=@ScoreId";
				SqlParameter[] para = new SqlParameter[]
				{
					new SqlParameter("@ScoreId",entity.ScoreId),
