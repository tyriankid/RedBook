...
namespace Taotaole.Dal
{
	...
	/// -数据库操作类
	/// </summary>
	public class go_moneyPoolDetailManager
	{
		...
		{
			string selectSql = string.Format(@"Select * From go_moneyPoolDetail Where id={0}", ID);
			DataSet ds = DataAccessFactory.GetDataProvider(DbServers.GetCurrentDB(currDbName)).GetDataset(selectSql);
			ds.Tables[0].TableName = "go_moneyPoolDetail";
		...
		public static go_moneyPoolDetailEntity LoadEntity(int ID, DbServers.DbServerName currDbName = DbServers.DbServerName.LatestDB)
		{
			...
			{
				return ReaderConvert.ReaderToModel<go_moneyPoolDetailEntity>(reader);
			}
		}
		...
		public static DataTable SelectListData(string where = null, string selectFields ="*", string orderby = null, int top = 0, DbServers.DbServerName currDbName = DbServers.DbServerName.LatestDB)
		{
			...
			string selectSql = string.Format(@"Select {2} {1} From go_moneyPoolDetail {0}", where, selectFields, top == 0 ? "" : "top " + top);
		...
		{
			if (!string.IsNullOrEmpty(where)) where = " Where " + where;
			string selectSql = string.Format(@"Select {1} From go_moneyPoolDetail {0}", where, selectFields);
			if (!string.IsNullOrEmpty(orderby)) selectSql += " Order By " + orderby;
		...
		/// 根据条件查询数据实体
		/// </summary>
		public static IList<go_moneyPoolDetailEntity> SelectListEntity(string where = null, string selectFields ="*", string orderby = null, int top = 0, DbServers.DbServerName currDbName = DbServers.DbServerName.LatestDB)
		{
			if (!string.IsNullOrEmpty(where)) where = " Where " + where;
			string selectSql = string.Format(@"Select {2} {1} From go_moneyPoolDetail {0}", where, selectFields, top == 0 ? "" : "top " + top);
			if (!string.IsNullOrEmpty(orderby)) selectSql += " Order By " + orderby;
			using (IDataReader reader = DataAccessFactory.GetDataProvider(DbServers.GetCurrentDB(currDbName)).GetReader(selectSql))
			{
				return ReaderConvert.ReaderToList<go_moneyPoolDetailEntity>(reader);
			}
		}
		...
		{
			string deleteSql = string.Format(@"Delete From go_moneyPoolDetail Where id={0}", ID);
		...
		{
			if (!string.IsNullOrEmpty(where)) where = " Where " + where;
			string deleteSql = string.Format(@"Delete From go_moneyPoolDetail {0}", where);
			DataAccessFactory.GetDataProvider(DbServers.GetCurrentDB(currDbName)).Execute(deleteSql);
		}
		...
		public static bool SaveEntity(go_moneyPoolDetailEntity entity, bool isAdd, DbServers.DbServerName currDbName = DbServers.DbServerName.LatestDB)
		{
			try
			{
                ...
                "Insert Into go_moneyPoolDetail(uid,money,addtime,moneyType,orderid)values(@uid,@money,@addtime,@moneyType,@orderid)" :
                "Update go_moneyPoolDetail Set uid=@uid,money=@money,addtime=@addtime,moneyType=@moneyType,orderid=@orderid  Where id=@id";
				SqlParameter[] para = new SqlParameter[]
				{
					...
					(entity.MoneyType==null)?new SqlParameter("@moneyType",DBNull.Value):new SqlParameter("@moneyType",entity.MoneyType),
				};
