...
namespace Taotaole.Dal
{
	...
	/// -数据库操作类
	/// </summary>
	public class RB_User_FabulousManager
	{
		...
		{
			string selectSql = string.Format(@"Select * From RB_User_Fabulous Where Id='{0}'", ID);
			DataSet ds = DataAccessFactory.GetDataProvider(DbServers.GetCurrentDB(currDbName)).GetDataset(selectSql);
			ds.Tables[0].TableName = "RB_User_Fabulous";
		...
		/// 根据主键查询数据实体
		/// </summary>
		public static RB_User_FabulousEntity LoadEntity(Guid ID, DbServers.DbServerName currDbName = DbServers.DbServerName.LatestDB)
		{
			string selectSql = string.Format(@"Select * From RB_User_Fabulous Where Id='{0}'", ID);
			using (IDataReader reader = DataAccessFactory.GetDataProvider(DbServers.GetCurrentDB(currDbName)).GetReader(selectSql))
			{
				return ReaderConvert.ReaderToModel<RB_User_FabulousEntity>(reader);
			}
		}
		...
		public static DataTable SelectListData(string where = null, string selectFields ="*", string orderby = null, int top = 0, DbServers.DbServerName currDbName = DbServers.DbServerName.LatestDB)
		{
			...
			string selectSql = string.Format(@"Select {2} {1} From RB_User_Fabulous {0}", where, selectFields, top == 0 ? "" : "top " + top);
		...
		{
			if (!string.IsNullOrEmpty(where)) where = " Where " + where;
			string selectSql = string.Format(@"Select {1} From RB_User_Fabulous {0}", where, selectFields);
			if (!string.IsNullOrEmpty(orderby)) selectSql += " Order By " + orderby;
		...
		/// 根据条件查询数据实体
		/// </summary>
		public static IList<RB_User_FabulousEntity> SelectListEntity(string where = null, string selectFields ="*", string orderby = null, int top = 0, DbServers.DbServerName currDbName = DbServers.DbServerName.LatestDB)
		{
			if (!string.IsNullOrEmpty(where)) where = " Where " + where;
			string selectSql = string.Format(@"Select {2} {1} From RB_User_Fabulous {0}", where, selectFields, top == 0 ? "" : "top " + top);
			if (!string.IsNullOrEmpty(orderby)) selectSql += " Order By " + orderby;
			using (IDataReader reader = DataAccessFactory.GetDataProvider(DbServers.GetCurrentDB(currDbName)).GetReader(selectSql))
			{
				return ReaderConvert.ReaderToList<RB_User_FabulousEntity>(reader);
			}
		}
		...
		{
			string deleteSql = string.Format(@"Delete From RB_User_Fabulous Where Id='{0}'", ID);
		...
		{
			if (!string.IsNullOrEmpty(where)) where = " Where " + where;
			string deleteSql = string.Format(@"Delete From RB_User_Fabulous {0}", where);
			DataAccessFactory.GetDataProvider(DbServers.GetCurrentDB(currDbName)).Execute(deleteSql);
		}
		...
		/// 保存数据
		/// </summary>
		public static bool SaveEntity(RB_User_FabulousEntity entity, bool isAdd, DbServers.DbServerName currDbName = DbServers.DbServerName.LatestDB)
		{
			try
			{
				string execSql = (isAdd) ?
				"Insert Into RB_User_Fabulous(Id,BusinessId,UserId,Status,AddTime,BusinessType)values(@Id,@BusinessId,@UserId,@Status,@AddTime,@BusinessType)" :
				"Update RB_User_Fabulous Set Id=@Id,BusinessId=@BusinessId,UserId=@UserId,Status=@Status,AddTime=@AddTime,BusinessType=@BusinessType Where Id=@Id";
